...
function PrimaryBtn({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <button className={`inline-flex items-center gap-1.5 rounded-md bg-primary px-3.5 py-2 text-sm font-medium text-primary-foreground shadow-xs transition-colors hover:opacity-90 ${className}`}>
      {children}
    </button>
  );
}
...
function GhostBtn({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <button className={`inline-flex items-center gap-1.5 rounded-md border border-border bg-background px-3.5 py-2 text-sm font-medium text-foreground transition-colors hover:bg-muted ${className}`}>
      {children}
    </button>
  );
}
    ...
    <section className="relative overflow-hidden border-b border-border">
      ...
      <div className={`${container} grid items-center gap-12 pt-20 pb-24 md:grid-cols-[1.05fr_0.95fr] md:pt-28 md:pb-32`}>
        <div className="space-y-7 animate-fade-up">
          ...
          <h1 className="text-4xl font-semibold leading-[1.05] tracking-tight md:text-6xl" style={display}>
  ...
  return (
    <section className="border-b border-border">
      <div className={`${container} grid grid-cols-2 divide-x divide-border md:grid-cols-4`}>
        {items.map((i) => (
          <div key={i.l} className="px-6 py-8 first:pl-0 last:pr-0">
  ...
  return (
    <div className={`grid items-center gap-12 md:grid-cols-2 md:gap-20 ${reverse ? "md:[&>*:first-child]:order-2" : ""}`}>
      <div className="space-y-5">
        ...
        <h2 className="text-3xl font-semibold tracking-tight md:text-4xl" style={display}>{title}</h2>
...
function MockPanel() {
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-card elev-sm">
...
function Modules() {
  ...
  return (
    <section id="modules" className="border-b border-border py-24 md:py-32">
      <div className={container}>
        ...
        <div className="grid grid-cols-1 border-t border-border sm:grid-cols-2 lg:grid-cols-4">
          {mods.map((m, i) => (
            <div
              key={m.name}
              className={`group border-b border-border p-6 transition-colors hover:bg-muted/40
                ${i % 4 !== 3 ? "lg:border-r" : ""} ${i % 2 !== 1 ? "sm:border-r lg:[&:nth-child(4n)]:border-r-0" : ""}`}
            >
              <m.icon className="h-4 w-4 text-muted-foreground transition-colors group-hover:text-accent" strokeWidth={1.75} />
  ...
  return (
    <section className="border-b border-border py-24 md:py-32">
      <div className={container}>
        ...
        <div className="overflow-hidden rounded-lg border border-border bg-card elev-md">
...
function Precision() {
  return (
    <section className="relative flex min-h-[520px] items-center justify-center overflow-hidden border-b border-border py-24">
      <div className="pointer-events-none absolute inset-0 opacity-40 dark:opacity-70">
        <PrecisionCanvas />
      </div>
      <div className="relative z-10 mx-auto max-w-2xl space-y-4 px-6 text-center">
...
function Testimonials() {
  ...
  return (
    <section className="overflow-hidden border-b border-border py-24 md:py-32">
      <div className={`${container} mb-12 max-w-2xl space-y-3`}>
      ...
      <div className="relative">
        ...
        <div className="flex w-max gap-4 animate-marquee">
          {doubled.map((t, i) => (
            <figure key={i} className="w-[380px] shrink-0 rounded-lg border border-border bg-card p-6">
              <blockquote className="text-sm leading-relaxed text-foreground">"{t.quote}"</blockquote>
              <figcaption className="mt-5 flex items-center gap-3">
                <div className="h-8 w-8 rounded-full border border-border bg-cover bg-center" style={{ backgroundImage: `url('${t.avatar}')` }} />
