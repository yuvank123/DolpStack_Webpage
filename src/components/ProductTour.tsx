...
import {
  LayoutDashboard, LayoutGrid, Bug, Languages, PenTool,
  Search, Bookmark, TrendingUp, Star, Clock,
  MessageSquare, CheckCircle2, AlertCircle, GitBranch,
  FileText, Globe, Plus, StickyNote, Share2, Rocket,
} from "lucide-react";
          ...
          {TABS.map((t) => {
            ...
            return (
              ...
              >
                ...
                <Icon className={`relative h-4 w-4 transition-opacity ${isActive ? "opacity-100" : "opacity-60 group-hover:opacity-100"}`} />
    ...
    <motion.div variants={staggerParent} initial="initial" animate="animate" className="space-y-5">
      <motion.div variants={staggerChild} className="flex items-center justify-between">
        <div>
          ...
          <div className="text-lg font-semibold tracking-tight text-foreground">Developer Workspace</div>
        </div>
        ...
        <Card>
          ...
          <ul className="space-y-2.5 text-sm">
            {[
              ...
              >
                <span className="text-muted-foreground"><span className="text-foreground">{k}</span> {v}</span>
  ...
  return (
    <motion.div variants={staggerParent} initial="initial" animate="animate" className="space-y-5">
      <motion.div variants={staggerChild} className="flex items-center gap-3">
        ...
        <button className="rounded-md border border-border bg-background px-3 py-2 text-xs text-foreground hover:border-foreground/30">All</button>
      </motion.div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <Card>
          ...
          <ul className="space-y-2 text-sm">
            {["react.dev — Suspense patterns", "vercel.com — Edge streaming", "kubernetes.io — HPA guide", "openai.com — Function calling"].map((r) => (
        ...
        <Card>
          ...
          <ul className="space-y-2 text-sm">
            {["tanstack.com/router", "supabase.com/docs/auth", "postgresql.org/docs/16", "tailwindcss.com/v4"].map((r) => (
              <li key={r} className="flex items-center justify-between text-muted-foreground">
                <span>{r}</span>
...
function DebugPage() {
  const bugs = [
    { id: "BUG-482", title: "Memory leak in React Dashboard", status: "Open", contrib: "amelia", messages: 12, active: true },
    { id: "BUG-471", title: "JWT refresh loop on expiry", status: "In review", contrib: "kenji", messages: 8, active: false },
    { id: "BUG-465", title: "Docker build failing on arm64", status: "Resolved", contrib: "priya", messages: 5, active: false },
  ];
  return (
    <motion.div variants={staggerParent} initial="initial" animate="animate" className="grid gap-4 md:grid-cols-[1fr_1.2fr]">
        ...
        {bugs.map((b) => (
          ...
          >
            ...
            <div className="mt-2 flex items-center gap-3 text-[11px] text-muted-foreground">
              <span className="inline-flex items-center gap-1"><MessageSquare className="h-3 w-3" /> {b.messages}</span>
              <span className="inline-flex items-center gap-1"><GitBranch className="h-3 w-3" /> @{b.contrib}</span>
            </div>
          </div>
        ))}
      </motion.div>
      ...
      <Card className="!p-5">
        ...
        <div className="mt-0.5 text-base font-semibold text-foreground">Memory leak in React Dashboard</div>
        <div className="mt-4 space-y-3 text-sm">
          <Section title="Description">
            Memory grows ~40MB/hr on the analytics view when live sockets are open.
          </Section>
          <Section title="Root cause">
            Chart component subscribes to a socket in <code className="rounded bg-muted px-1 py-0.5 text-[11px]">useEffect</code> without a cleanup function.
          </Section>
          ...
          <Section title="Resolved solution">
            <div className="mt-1 flex items-center gap-2 text-[var(--success)]">
              <CheckCircle2 className="h-3.5 w-3.5" /> Return unsubscribe from effect + weak-ref cache
            </div>
          </Section>
        </div>
      </Card>
...
function DocsPage() {
  return (
    <motion.div variants={staggerParent} initial="initial" animate="animate" className="grid gap-4 md:grid-cols-[200px_1fr]">
      <motion.div variants={staggerChild} className="space-y-3">
        ...
        <ul className="space-y-1 text-sm">
          {["Getting started", "Authentication", "Realtime sync", "Voice API", "Deployments"].map((d, i) => (
            <li key={d} className={`flex items-center gap-2 rounded px-2 py-1 ${i === 2 ? "bg-muted text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
      ...
      <motion.div variants={staggerChild} className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-lg font-semibold tracking-tight text-foreground">Realtime sync</div>
            <div className="text-[11px] text-muted-foreground">Updated 2 days ago · Translated from English</div>
          </div>
          <span className="rounded-full border border-border px-2 py-0.5 text-[10px] text-muted-foreground">AI-translated</span>
        </div>
        <Card className="!p-4">
          <pre className="overflow-x-auto text-[12px] leading-relaxed text-foreground/85"><code>{`import { sync } from "@dolpstack/realtime";
...
sync.channel("workspace:42").on("update", (msg) => {
  console.log(msg.payload);
        ...
        <div className="text-sm leading-relaxed text-muted-foreground">
          Channels multiplex over a single websocket. Subscribers automatically
          reconcile missed events on reconnect, ensuring every collaborator sees
          the same state within ~40ms.
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          <Card>
            <div className="mb-2 text-[11px] uppercase tracking-wider text-muted-foreground">Recently opened</div>
          ...
          <Card>
            <div className="mb-2 text-[11px] uppercase tracking-wider text-muted-foreground">Suggested</div>
            <ul className="space-y-1.5 text-sm text-muted-foreground">
...
function CanvasPage() {
  const notes = [
    { x: 6, y: 8, color: "bg-[oklch(0.92_0.08_85)] text-[oklch(0.25_0.05_85)]", text: "Client requests /api" },
    { x: 40, y: 18, color: "bg-[oklch(0.9_0.09_180)] text-[oklch(0.25_0.05_200)]", text: "Edge gateway routes + auth" },
    { x: 72, y: 10, color: "bg-[oklch(0.9_0.08_320)] text-[oklch(0.25_0.05_320)]", text: "Fan-out to workers" },
    { x: 20, y: 62, color: "bg-[oklch(0.92_0.08_140)] text-[oklch(0.25_0.05_140)]", text: "Postgres primary" },
    { x: 60, y: 66, color: "bg-[oklch(0.92_0.09_60)] text-[oklch(0.25_0.05_60)]", text: "Cache layer (Redis)" },
  ];
  return (
    <motion.div variants={staggerParent} initial="initial" animate="animate" className="space-y-4">
      <motion.div variants={staggerChild} className="flex items-center justify-between">
        <div>
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Glassboard</div>
      ...
      >
        <svg className="absolute inset-0 h-full w-full text-muted-foreground/50" viewBox="0 0 100 100" preserveAspectRatio="none">
          <motion.line initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.8, ease: EASE }} x1="15" y1="14" x2="45" y2="24" stroke="currentColor" strokeWidth="0.3" strokeDasharray="1 1" />
          <motion.line initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.8, delay: 0.15, ease: EASE }} x1="55" y1="24" x2="76" y2="16" stroke="currentColor" strokeWidth="0.3" strokeDasharray="1 1" />
          <motion.line initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.8, delay: 0.3, ease: EASE }} x1="50" y1="30" x2="30" y2="66" stroke="currentColor" strokeWidth="0.3" strokeDasharray="1 1" />
          <motion.line initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.8, delay: 0.45, ease: EASE }} x1="52" y1="30" x2="65" y2="70" stroke="currentColor" strokeWidth="0.3" strokeDasharray="1 1" />
        </svg>
        {notes.map((n, i) => (
          <motion.div
            key={n.text}
